...
This file defines stable work IDs, dependencies, and acceptance boundaries; it is not a
manually maintained completion dashboard. The
[GitHub issue tracker](https://github.com/quarj0/IdentityCore/issues) is authoritative for
open/closed state, and each closed issue must link to its merged pull request and
verification evidence. If tracker state and repository behavior disagree, reopen the issue
or create a focused reconciliation issue rather than adding a second status list here.
