...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (path[0] === "sessions" && path[1] && path[1] !== session.sessionId) {
    return NextResponse.json({ error: { message: "Session identifier mismatch." } }, { status: 403 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: { message: "Verification service unavailable." } }, { status: 502 });
  } finally {
