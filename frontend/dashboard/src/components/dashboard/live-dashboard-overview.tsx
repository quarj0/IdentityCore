...
export function LiveDashboardOverview() {
    ...
    void (async () => {
      try {
        ...
        const [me, organization, tenant, verifications] = await Promise.all([
          ...
          backend.rest<{ pagination: { total: number } }>(
            "/verifications/?page=1&page_size=1",
          ),
        ]);
        setData({
          user: me.user,
          organization,
          tenant,
          verificationCount: verifications.pagination.total,
        });
      } catch (caught) {
        setError(
    ...
    return (
      <Card className="rounded-3xl border-red-200 bg-red-50">
        <CardContent className="p-8">
          <h1 className="font-semibold text-red-800">Workspace unavailable</h1>
          <p className="mt-2 text-sm text-red-700">{error}</p>
          <a
            className="mt-4 inline-block text-sm font-medium text-blue-700"
            href={`${process.env.NEXT_PUBLIC_IDENTITYCORE_ORIGIN ?? "http://localhost:3001"}/login`}
          >
            Sign in again
          </a>
        </CardContent>
  ...
  const cards = [
    {
      ...
      detail: data.organization.status,
      icon: Building2,
    },
    {
      label: "Workspace",
      value: data.tenant.name,
      detail: data.tenant.status,
      icon: Users,
    },
    {
      label: "Verifications",
      value: String(data.verificationCount),
      detail: "Tenant-scoped total",
      icon: Activity,
    },
    {
      label: "Access",
      value: data.user.roles[0] ?? "Member",
  ...
  return (
    <div className="space-y-8">
      <PageHeading
        title={`Welcome, ${data.user.first_name || data.organization.name}`}
        description="Your live IdentityCore organization workspace."
      />
