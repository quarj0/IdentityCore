...
export function LiveProjectDetailPage({
  id,
  settings = false,
}: {
  id: string;
  ...
  async function toggleStatus() {
    ...
    setBusy("status");
    try {
      await dashboardApi.projectAction(
        id,
        project.status === "suspended" ? "reactivate" : "suspend",
      );
      await load();
      setMessage(
  ...
  return (
    <div className="space-y-8">
      <PageHeading
        title={settings ? "Project settings" : (project?.name ?? "Project")}
          ...
          <Card>
            <CardContent className="grid gap-5 p-6 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-1 capitalize">{project.environment}</p>
              </div>
              <div>
          ...
          <Card>
            <CardContent className="space-y-5 p-6">
              ...
              <div className="flex flex-wrap gap-3">
                <Button onClick={save} disabled={!name.trim() || Boolean(busy)}>
                  {busy === "save" ? "Saving..." : "Save settings"}
                </Button>
                <Button variant="outline" asChild>
                  <Link href={`/workflows?project=${project.id}`}>
                    View workflows
                  </Link>
                </Button>
                <Button
