...
export function LiveApiKeysPage() {
  ...
  useEffect(() => {
    dashboardApi
      .organization()
      .then((organization) =>
        setPendingApproval(organization.status !== "active"),
      )
      .catch(() => undefined);
  }, []);
  useEffect(() => {
  ...
  async function keyAction(item: APIClient, action: "rotate" | "revoke") {
    ...
    try {
      ...
      if (action === "rotate") {
        ...
        setSecret("");
        setMessage(
          "API key revoked. You can create a replacement sandbox key.",
        );
      }
      await load();
    } catch (caught) {
  ...
  return (
    <div className="space-y-8">
      ...
      {pendingApproval && hasUsableKey ? (
        <Card className="rounded-2xl border-amber-200 bg-amber-50">
          <CardContent className="p-5 text-sm text-amber-900">
            <p className="font-semibold">Sandbox test-key limit reached</p>
            <p className="mt-1">
              Pending workspaces can keep one test key with verification
              read/create scopes and a 30 requests-per-minute limit. Rotate it
              if the secret was missed, or revoke it to create a replacement.
            </p>
          </CardContent>
        </Card>
      ) : (
            ...
            <form onSubmit={create} className="grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Sandbox keys are available now. Production keys require
                  platform approval.
                </p>
              </div>
              <div>
                  ...
                  {item.status !== "revoked" ? (
                    <>
                      <Button
                        size="sm"
                        variant="outline"
                        disabled={acting === item.public_id}
                        onClick={() => keyAction(item, "rotate")}
                      >
                        Rotate secret
                      </Button>
                      <Button
