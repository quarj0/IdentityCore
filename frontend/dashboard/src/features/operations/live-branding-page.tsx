...
export function LiveBrandingPage() {
  ...
  useEffect(() => {
    dashboardApi
      .organization()
      .then((organization) =>
        setLogoUrl(String(organization.settings.logo_url ?? "")),
      );
  }, []);
  ...
  async function upload() {
    ...
    try {
      ...
      setLogoUrl(String(organization.settings.logo_url ?? target.asset_url));
      setMessage("Branding logo saved.");
    } catch (error) {
      setMessage(
  ...
  return (
    <div className="space-y-8">
      <PageHeading
        title="Branding"
        description="Customize the logo shown in hosted verification experiences."
      />
      <Card>
        <CardContent className="space-y-6 p-6">
          {logoUrl ? (
            ...
            <div className="flex h-20 w-64 items-center justify-center rounded-xl border border-dashed text-sm text-slate-500">
              No logo uploaded
            </div>
          )}
          <div>
