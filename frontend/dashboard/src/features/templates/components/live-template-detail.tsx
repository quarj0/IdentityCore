    ...
    try {
      const result = await dashboardApi.patchPolicy(item.id, {
        ...
        required_document_types: String(
          data.get("required_document_types") || "",
        )
          .split(",")
          .map((value) => value.trim())
          .filter(Boolean),
        required_liveness_level: String(
          data.get("required_liveness_level") || "passive",
        ),
        face_match_threshold: String(
          data.get("face_match_threshold") || "0.8500",
        ),
        ...
        metadata_retention_days: Number(
          data.get("metadata_retention_days") || 365,
        ),
      });
      setItem(result);
  ...
  return (
    <div className="space-y-8">
      <PageHeading
        ...
        description={
          editable
            ? "Draft templates can be edited before activation."
            : "Active and archived template versions are immutable. Clone to create a new draft."
        }
        ...
        <CardContent>
          <form onSubmit={save} className="grid gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <Input
                id="required_document_types"
                name="required_document_types"
                defaultValue={item.required_document_types.join(", ")}
                disabled={!editable}
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                ...
                defaultValue={item.face_match_threshold.toFixed(4)}
                disabled={!editable}
              />
            </div>
            <div>
              ...
              <Input
                ...
                defaultValue={item.manual_review_threshold.toFixed(4)}
                disabled={!editable}
              />
            </div>
            ...
            <div>
              ...
              <Input
                id="verification_expiry_minutes"
                name="verification_expiry_minutes"
                type="number"
                defaultValue={item.verification_expiry_minutes}
                disabled={!editable}
              />
            </div>
            <div>
              ...
              <Input
                ...
                type="number"
                defaultValue={item.media_retention_days}
                disabled={!editable}
              />
            </div>
            <div>
              <Label htmlFor="metadata_retention_days">
                Metadata retention days
              </Label>
              <Input
                id="metadata_retention_days"
                name="metadata_retention_days"
                type="number"
                defaultValue={item.metadata_retention_days}
                disabled={!editable}
              />
            </div>
            {editable ? (
