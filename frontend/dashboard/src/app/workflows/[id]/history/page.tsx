...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([dashboardApi.workflow(id), dashboardApi.workflowVersions(id)])
      .then(([current, history]) => {
        setWorkflow(current);
        setVersions(history.results as Version[]);
      })
      .catch((caught) =>
        setError(
  ...
  return (
    <div className="space-y-8">
      <PageHeading
        ...
        description="Draft and immutable published versions"
      />
      {error ? (
        ...
        <div className="grid gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    ...
                    {workflow.current_version === 0
                      ? "0 (unpublished)"
                      : `${workflow.current_version + 1} draft`}{" "}
          ...
          {versions.length ? (
            ...
            <p className="text-sm text-slate-500">
              No published versions yet. Pending workspaces can edit this draft;
