...
class LiveCompatibilityTest {
    @Test void createGetListErrorAndWebhook() throws Exception {
        ...
        byte[] payload = "{\"type\":\"verification.completed\"}".getBytes(StandardCharsets.UTF_8); String timestamp = Long.toString(Instant.now().getEpochSecond()); String key = "webhook-secret";
        Mac mac = Mac.getInstance("HmacSHA256"); mac.init(new SecretKeySpec(key.getBytes(StandardCharsets.UTF_8), "HmacSHA256")); mac.update((timestamp + ".").getBytes(StandardCharsets.UTF_8));
