...
ENVIRONMENTS = {
    "sandbox": "https://api.identitycore.com",
...
COMMAND_OPTIONS = {
    ("verifications", "create"): (
        "--purpose --policy-id --full-name --email --project-id "
        "--external-reference --redirect-url --idempotency-key"
    ),
...
def _profile(config: dict[str, Any], name: str) -> dict[str, str]:
    ...
    return {
        ...
        for key in ("api_origin", "client_id", "client_secret")
    }


def _client(args: argparse.Namespace) -> IdentityCoreClient:
    config = _profile(_load_config(), args.profile)
    explicit_environment = args.environment or os.environ.get(
        "IDENTITYCORE_ENVIRONMENT"
    )
    environment = explicit_environment or config.get("environment", "")
    origin = (
        args.api_origin
        or os.environ.get("IDENTITYCORE_API_ORIGIN")
        or (ENVIRONMENTS.get(explicit_environment, "") if explicit_environment else "")
        or config.get("api_origin", "")
        or ENVIRONMENTS.get(environment, "")
    )
    return IdentityCoreClient(
        api_origin=origin,
        client_id=args.client_id
        or os.environ.get("IDENTITYCORE_CLIENT_ID")
        or config.get("client_id", ""),
        client_secret=args.client_secret
        or os.environ.get("IDENTITYCORE_CLIENT_SECRET")
        or config.get("client_secret", ""),
        access_token=args.access_token
        or os.environ.get("IDENTITYCORE_ACCESS_TOKEN")
...
def _print(value: Any, output: str = "json") -> None:
    ...
    widths = {
        key: max(len(key), *(len(_scalar(row.get(key))) for row in rows))
        for key in columns
    }
...
Documentation: https://docs.identitycore.com/cli""",
    ...
    parser.add_argument(
        "--profile",
        default=os.environ.get("IDENTITYCORE_PROFILE", "default"),
        help="Configuration profile (default: default)",
    )
    parser.add_argument(
        "--environment",
        choices=tuple(ENVIRONMENTS),
        help="Use the sandbox or production API origin",
    )
    ...
    parser.add_argument(
        "--client-secret", help="API client secret, or IDENTITYCORE_CLIENT_SECRET"
    )
    parser.add_argument(
        "--access-token", help="User access token, or IDENTITYCORE_ACCESS_TOKEN"
    )
    parser.add_argument(
        "--output",
        choices=("json", "table"),
        default=os.environ.get("IDENTITYCORE_OUTPUT", "json"),
    )
    parser.add_argument(
        "--version", action="version", version=f"IdentityCore CLI {__version__}"
    )
    ...
    pl = policies.add_parser("list")
    _list_flags(pl)
    pg = policies.add_parser("get")
    pg.add_argument("policy_id")

    verifications = _resource(
        subs, "verifications", "Manage verifications and evidence"
    )
    vl = verifications.add_parser("list")
    _list_flags(vl)
    vl.add_argument("--external-reference")
    vg = verifications.add_parser("get")
    vg.add_argument("verification_id")
    vc = verifications.add_parser("cancel")
    vc.add_argument("verification_id")
    vc.add_argument("--reason", default="")
    ...
    ve.add_argument("verification_id")
    ve.add_argument("--file", required=True)
    ve.add_argument("--pdf", action="store_true")

    projects = _resource(subs, "projects", "Manage projects and environments")
    pjl = projects.add_parser("list")
    pjl.add_argument("--page", type=int)
    pjl.add_argument("--page-size", type=int)
    pjl.add_argument("--status")
    pjl.add_argument("--environment", choices=tuple(ENVIRONMENTS))
    pjg = projects.add_parser("get")
    pjg.add_argument("project_id")
    pjc = projects.add_parser("create")
    pjc.add_argument("--name", required=True)
    pjc.add_argument("--environment", choices=tuple(ENVIRONMENTS), required=True)
    for action in ("enable", "disable"):
        p = projects.add_parser(action)
        p.add_argument("project_id")

    clients = _resource(subs, "api-clients", "Create, rotate, and revoke API clients")
    acl = clients.add_parser("list")
    _list_flags(acl)
    acg = clients.add_parser("get")
    acg.add_argument("api_client_id")
    acc = clients.add_parser("create")
    ...
    wl = workflows.add_parser("list")
    _list_flags(wl, status=False)
    wl.add_argument("--project-id")
    wg = workflows.add_parser("get")
    wg.add_argument("workflow_id")
    wc = workflows.add_parser("create")
    wc.add_argument("--name", required=True)
    wc.add_argument("--project-id", required=True)
    wc.add_argument(
        "--definition", default="{}", help="Workflow fields as a JSON object"
    )
    for action in ("publish", "archive", "clone", "versions"):
        p = workflows.add_parser(action)
        p.add_argument("workflow_id")

    templates = _resource(subs, "templates", "Browse workflow templates")
    tl = templates.add_parser("list")
    _list_flags(tl, status=False)
    tl.add_argument("--category")
    tl.add_argument("--search")
    tg = templates.add_parser("get")
    tg.add_argument("template_id")

    webhooks = _resource(subs, "webhooks", "Manage and test webhook endpoints")
    whl = webhooks.add_parser("list")
    _list_flags(whl)
    whg = webhooks.add_parser("get")
    whg.add_argument("webhook_id")
    whc = webhooks.add_parser("create")
    whc.add_argument("--url", required=True)
...
def _completion_script(shell: str) -> str:
    ...
    if shell == "bash":
        resource_cases = "\n".join(
            f'        {name}) opts="{actions}" ;;' for name, actions in COMMANDS.items()
        )
        action_cases = "\n".join(
            f'      {resource}:{action}) opts="{options}" ;;'
            for (resource, action), options in COMMAND_OPTIONS.items()
        )
        return f"""# bash completion for identitycore
_identitycore_completion() {{
  local cur resource action opts index word
  cur="${{COMP_WORDS[COMP_CWORD]}}"
  for ((index=1; index<COMP_CWORD; index++)); do
    word="${{COMP_WORDS[index]}}"
    case "$word" in
      --profile|--environment|--api-origin|--client-id|--client-secret|--access-token|--output) ((index++)) ;;
      projects|api-clients|workflows|templates|webhooks|verifications|policies|completion)
        if [[ -z "$resource" ]]; then resource="$word"; fi ;;
      *) if [[ -n "$resource" && -z "$action" && "$word" != --* ]]; then action="$word"; fi ;;
    esac
  done
...
complete -F _identitycore_completion identitycore"""
    if shell == "zsh":
        descriptions = " ".join(
            f"'{name}:{name.replace('-', ' ')} commands'" for name in top.split()
        )
        action_cases = "\n".join(
            f"    {resource}:{action}) _values option {options} ;;"
            for (resource, action), options in COMMAND_OPTIONS.items()
        )
        resource_cases = "\n".join(
            f"      {name}) _values action {actions} ;;"
            for name, actions in COMMANDS.items()
        )
        return f"""#compdef identitycore
_identitycore() {{
  local -a commands
  local resource action word
  local -i index
  ...
  for ((index=2; index<CURRENT; index++)); do
    ...
    case $word in
      --profile|--environment|--api-origin|--client-id|--client-secret|--access-token|--output) ((index++)) ;;
      projects|api-clients|workflows|templates|webhooks|verifications|policies|completion)
        [[ -z $resource ]] && resource=$word ;;
      *) [[ -n $resource && -z $action && $word != --* ]] && action=$word ;;
    esac
  done
  case "$resource:$action" in
{action_cases}
...
compdef _identitycore identitycore"""
    ...
    lines += [
        f"complete -c identitycore -n '__fish_use_subcommand' -a '{name}'"
        for name in top.split()
    ]
    for name, actions in COMMANDS.items():
        lines.append(
            f"complete -c identitycore -n '__fish_seen_subcommand_from {name}' -a '{actions}'"
        )
    lines.append(
        "complete -c identitycore -n '__fish_seen_subcommand_from completion' "
        "-a 'bash zsh fish'"
    )
    for (resource, action), options in COMMAND_OPTIONS.items():
        for option in options.split():
            lines.append(
                f"complete -c identitycore -n '__fish_seen_subcommand_from {resource}; "
                f"and __fish_seen_subcommand_from {action}' -l {option.removeprefix('--')}"
            )
    lines += [
        "complete -c identitycore -l profile -r",
        "complete -c identitycore -l environment -a 'sandbox production'",
        "complete -c identitycore -l output -a 'json table'",
    ]
    return "\n".join(lines)
...
def _dispatch(client: IdentityCoreClient, args: argparse.Namespace) -> Any:
    ...
    if command == "policies":
        return client.request(
            "GET",
            "/policies/" + (_pagination(args) if action == "list" else args.policy_id),
        )
    if command == "verifications":
        ...
        if action == "get":
            return client.verifications.retrieve(args.verification_id)
        if action == "cancel":
            return client.verifications.cancel(args.verification_id, reason=args.reason)
        if action == "create":
            return client.verifications.create(
        ...
        data = client.download(
            f"/verifications/{args.verification_id}/evidence-report{suffix}"
        )
        path = Path(args.file)
        path.parent.mkdir(parents=True, exist_ok=True)
        ...
        return {
            "file": str(path),
            "bytes": len(data),
            "format": "pdf" if args.pdf else "archive",
        }
    bases = {
        "projects": "/projects/",
    ...
    if action == "get":
        return client.request("GET", base + identifier)
    if action == "create":
        if command == "projects":
            body = {"name": args.name, "environment": args.environment}
        elif command == "api-clients":
            body = {
                "name": args.name,
                "project_id": args.project_id,
                "scopes": args.scopes,
            ...
            body = {
                **_json_object(args.definition, "--definition"),
                "name": args.name,
                "project_id": args.project_id,
            }
        else:
    ...
    if command == "webhooks" and action == "test":
        return client.request("POST", f"{base}{identifier}/test", {})
    mapped_action = {
        ("projects", "disable"): "suspend",
        ("projects", "enable"): "activate",
    }.get((command, action), action)
    return client.request("POST", f"{base}{identifier}/{mapped_action}", {})


def _handle_profiles(args: argparse.Namespace) -> Any:
    config = _load_config()
    profiles = (
        config.get("profiles", {}) if isinstance(config.get("profiles"), dict) else {}
    )
    ...
    if action == "list":
        return {
            ...
            "results": [
                {
                    "name": name,
                    "environment": value.get("environment", "custom"),
                    "api_origin": value.get("api_origin", ""),
                }
    ...
    if action == "current":
        ...
        visible = {
            ...
            if key not in {"client_secret", "access_token"}
        }
        return {
            "name": args.profile,
            **visible,
            "authenticated": bool(
                current.get("client_secret") or current.get("access_token")
            ),
        }
        ...
        if args.command == "login":
            ...
            if not token and (not args.client_id or not secret):
                raise IdentityCoreError(
                    "--access-token or --client-id and --client-secret are required (environment variables are recommended in CI)."
                )
            config = _load_config()
            profiles = (
                config.get("profiles", {})
            ...
            if not profiles and any(
                config.get(key)
                for key in ("api_origin", "client_id", "client_secret", "access_token")
            ):
                profiles["default"] = {
                    key: str(config.get(key, ""))
                    for key in (
            ...
            profiles[args.profile] = {
                "environment": args.environment or "custom",
                "api_origin": args.api_origin or ENVIRONMENTS[args.environment],
            ...
            _save_config(config)
            _print(
                {"profile": args.profile, "configuration": str(_config_path())},
                args.output,
            )
            return 0
        if args.command == "profiles":
            _print(_handle_profiles(args), args.output)
            return 0
        _print(_dispatch(_client(args), args), args.output)
        return 0
