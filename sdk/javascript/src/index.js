...
export class IdentityCoreClient {
  constructor({
    ...
    try {
      ...
      throw new IdentityCoreError("apiOrigin must be an absolute HTTP(S) URL.");
    }
    if (!/^https?:/.test(apiOrigin))
      throw new IdentityCoreError("apiOrigin must be an absolute HTTP(S) URL.");
    if (!clientId || !clientSecret)
      throw new IdentityCoreError("clientId and clientSecret are required.");
    if (!fetchImpl)
      throw new IdentityCoreError("A fetch implementation is required.");
    if (timeout <= 0 || maxRetries < 0 || retryBackoff < 0)
      throw new IdentityCoreError(
        "timeout must be positive and retry settings cannot be negative.",
      );
    if (!allowBrowser && typeof window !== "undefined")
      throw new IdentityCoreError(
        "The IdentityCore secret-key SDK is server-side only. Do not expose clientSecret in browser code.",
      );
    ...
    this.policies = {
      ...
      retrieve: (id) => this.request("GET", `/policies/${id}`),
    };
    this.verifications = {
      create: (input, options = {}) =>
        this.request(
          ...
          "/verifications/",
          {
            ...
            verification_subject: subjectToApi(
              input.verificationSubject ?? input.verification_subject,
            ),
            external_reference:
              input.externalReference ?? input.external_reference ?? "",
      ...
      list: ({
        ...
        this.request(
          "GET",
          `/verifications/${compactQuery({ status, external_reference: externalReference, cursor, limit, page, page_size: pageSize })}`,
        ),
      iterate: async function* (options = {}) {
        let cursor = "";
        do {
      ...
      result: (id) =>
        this.request("GET", `/verifications/${encodeURIComponent(id)}/result`),
      cancel: (id, { reason = "" } = {}) =>
        this.request("POST", `/verifications/${id}/cancel`, { reason }),
      resendLink: (id, { channel = "email" } = {}) =>
        this.request("POST", `/verifications/${id}/resend-link`, { channel }),
      evidenceReport: (id) =>
        this.request("GET", `/verifications/${id}/evidence-report`),
    };
  }
  ...
  async request(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt += 1) {
      ...
      const headers = {
        ...
        "User-Agent": `identitycore-node/${VERSION}`,
        ...extraHeaders,
      };
      ...
      try {
        ...
        if (
          ...
          await delay(this.retryBackoff * 2 ** attempt);
          continue;
        }
        ...
        if (error?.name === "AbortError")
          throw new IdentityCoreTimeoutError(
            "The request timed out. Please try again.",
            { cause: error },
          );
        throw new IdentityCoreConnectionError(
          "Could not connect to IdentityCore.",
          { cause: error },
        );
      } finally {
